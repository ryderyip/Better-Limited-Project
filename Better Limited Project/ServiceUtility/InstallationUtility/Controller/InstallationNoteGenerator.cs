...
namespace Better_Limited_Project.ServiceUtility.InstallationUtility.Controller
{
    public class InstallationNoteGenerator
    {
        ...
        public InstallationNoteGenerator(Installation installation)
        {
            ...
            var date = _installation.ScheduledOn.ToString("yy-MMM-dd ddd");
            var orderNumber = installation.InstallationRequest.SalesOrder.OrderNumber;
            _fileName = $"installation note {orderNumber} ({date}).pdf";
        ...
        private Document Generate()
        {
            ...
            doc.Info.Title = "Installation Note";
        ...
        private void CreatePage(Document doc)
        {
            ...
            var paymentReceiptParagraph = headerText.AddParagraph("Installation Note");
            paymentReceiptParagraph.Format.Font = new Font("Arial", 24);

            // Add the print date field
            var dateTextFrame = section.AddTextFrame();
            dateTextFrame.Height = "2cm";
            dateTextFrame.Width = "10cm";
            var paragraph = dateTextFrame.AddParagraph();
            paragraph.Format.SpaceAfter = "1cm";
            paragraph.Format.Font = new Font("Arial", 9);
            string date = _installation.ScheduledOn.ToString("D");
            string time = _installation.ScheduledOn.ToShortTimeString();
            string orderNumber = _installation.InstallationRequest.SalesOrder.OrderNumber;
            ...
            row.Cells[1].AddParagraph("Product to Install");
            ...
            foreach (var installationProduct in _installation.InstallationRequest.ProductsToInstall)
            {
                ...
                row.Cells[0].Format.Alignment = ParagraphAlignment.Left;
                row.Cells[1].AddParagraph(installationProduct.Product.Name);
                row.Cells[1].Format.Alignment = ParagraphAlignment.Left;
                row.Cells[2].AddParagraph(installationProduct.Quantity.ToString());
                row.Cells[2].Format.Alignment = ParagraphAlignment.Left;
            }
