    ...
    public static class SalesOrderProductPaymentRepository
    {
        public static IEnumerable<SalesOrderProductPayment> GetByIds(string salesOrderId, string productId)
        {
            var command = new MySqlCommand(
                @"select payment_id, is_deposit from sales_order_product_payment
            ...
            return from DataRow row in datatable.Rows
                ...
                select new SalesOrderProductPayment(salesOrderId, productId, paymentId, isDeposit);
        }

        public static void Insert(SalesOrderProductPayment salesOrderProductPayment)
        {
            var command = new MySqlCommand(
                @"insert into sales_order_product_payment 
                    value (@salesOrderId, @productId, @paymentId, @isDeposit);");
        ...
        {
            var command = new MySqlCommand(
                @"delete from sales_order_product_payment where sales_order_id = @salesOrderId 
                                          and product_id = @productId 
